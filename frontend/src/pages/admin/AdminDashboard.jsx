...
function StatValue({ value, loading }) {
  if (loading) {
    return <div className="h-9 w-24 rounded-lg bg-[#1E2E48] animate-pulse mt-1" />;
  }
  return (
...
function ConsensusHealth({ status, loading }) {
  if (loading) {
    return <div className="h-9 w-28 rounded-lg bg-[#1E2E48] animate-pulse mt-1" />;
  }
    ...
    <div className="flex items-center gap-2 mt-1">
      <span
        className={`w-3 h-3 rounded-full shrink-0 ${
          isNominal ? 'bg-emerald-400 animate-pulse shadow-[0_0_8px_rgba(52,211,153,0.6)]' : 'bg-red-400'
        }`}
      />
      <span
        className={`text-[28px] font-black leading-none tabular-nums ${
          isNominal ? 'text-emerald-300' : 'text-red-300'
        }`}
      >
        {status ?? '—'}
      </span>
 ...
 * Route: /admin  and  /admin/identities
 * Guard: ProtectedRoute allowedRoles={["ADMIN"]} (set in App.jsx)
 */
export default function AdminDashboard() {
  ...
  /* Table refresh trigger — increment to cause IdentityTable to re-fetch */
  ...
  const fetchStats = useCallback(async () => {
    try {
      ...
      /* Silently tolerate — stats are non-critical display data */
      setStats(null);
    } finally {
    ...
    <>
      ...
      <div className="mb-7">
        ...
        <div className="flex items-start justify-between gap-4">
          ...
          {/* Live indicator */}
          <div className="flex items-center gap-2 shrink-0 mt-1 px-3 py-1.5 rounded-full bg-emerald-950/30 border border-emerald-700/20">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4 mb-8">
        {statCards.map((card) => (
          <Card key={card.id} goldAccent hoverable id={card.id} className="dashboard-card">
            <CardHeader>
              <CardTitle>{card.label}</CardTitle>
              <span
                className="material-symbols-outlined text-[20px] text-[#D4AF37]/60 shrink-0"
      ...
      <div className="relative">
        ...
        <div className="flex items-center gap-3 mb-5">
          ...
          <h2 className="text-[11px] font-black tracking-[0.2em] uppercase text-slate-400">
            Identity Management
          </h2>
          <span className="flex-1 h-px bg-[#1E2E48]" />
        </div>

        {/* IdentityTable handles its own data-fetching */}
        <IdentityTable
        ...
        <div className="flex justify-end mt-4">
          <button
            ...
            className="inline-flex items-center gap-2 rounded-lg bg-[#1E5FA8] px-4 py-2.5 text-[12px] font-bold text-white shadow-lg transition-colors hover:bg-[#174d8a]"
          >
      ...
      <CreateIdentityModal
        ...
        onCreated={() => {
          ...
          fetchStats(); // bump stats count immediately
        }}
      />
